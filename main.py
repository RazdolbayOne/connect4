...
def is_valid_location(board, col):
    return board[ROW_COUNT - 1][
               col] == 0  # Checking to make sure that the column has not been filled fully, it checks the last row only (it returns a boolean value)
    ...
    for c in range(COLUMN_COUNT):
        for r in range(ROW_COUNT):
            pygame.draw.rect(screen, BLUE, (c * SQUARESIZE, r * SQUARESIZE + SQUARESIZE, SQUARESIZE,
                                            SQUARESIZE))  # These are the (screen size,positon on y-axis,width,height)
            pygame.draw.circle(screen, BLACK, (
            int(c * SQUARESIZE + SQUARESIZE / 2), int(r * SQUARESIZE + SQUARESIZE + SQUARESIZE / 2)),
                               RADIUS)  # pygame accepts only integers (refer pygame documentation)
